...
from app.schemas.share_schema import CreateShareSchema, UpdateShareSchema
from app.services.share_service import ShareService

create_share_schema = CreateShareSchema()
update_share_schema = UpdateShareSchema()
...
@api_v1_blueprint.route("/notes/<note_id>/share/<share_id>", methods=["PUT"])
@jwt_required()
def update_share(note_id, share_id):
    ...
    try:
        data = update_share_schema.load(json_data)
    except ValidationError as err:
        return jsonify({
            "error": "VALIDATION_ERROR",
            "message": err.messages,
    ...
    try:
        share = ShareService.update_share_permission(
            share_id=share_id,
            note_id=note_id,
            owner_id=current_user_id,
            permission=data["permission"],
...
@api_v1_blueprint.route("/notes/<note_id>/shares", methods=["GET"])
@jwt_required()
def list_note_shares(note_id):

    current_user_id = get_jwt_identity()

    try:
        shares = ShareService.get_note_shares(
            note_id=note_id,
            owner_id=current_user_id,
        )
    except PermissionError as err:
